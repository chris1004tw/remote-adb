...
### 1. Signaling Server (`radb-signal`)
...
| 類別 | Type | 方向 | 說明 |
|------|------|------|------|
| 認證 | `auth` | Client/Agent → Signal | 傳送 PSK token 與角色 |
| 認證 | `auth_ack` | Signal → Client/Agent | 認證結果與分配的 ID |
| 主機 | `register` | Agent → Signal | Host 註冊（含設備列表） |
| 主機 | `unregister` | Agent → Signal | Host 取消註冊 |
| 查詢 | `host_list` | Client → Signal | 請求可用主機列表 |
| 查詢 | `host_list_resp` | Signal → Client | 回傳主機列表 |
| 設備 | `device_update` | Agent → Signal → Client | 設備狀態即時更新 |
| 鎖定 | `lock_req` | Client → Signal → Agent | 請求鎖定設備 |
| 鎖定 | `lock_resp` | Agent → Signal → Client | 鎖定結果 |
| 解鎖 | `unlock_req` | Client → Signal → Agent | 請求解鎖設備 |
| 解鎖 | `unlock_resp` | Agent → Signal → Client | 解鎖結果 |
| WebRTC | `offer` | Client → Signal → Agent | SDP Offer |
| WebRTC | `answer` | Agent → Signal → Client | SDP Answer |
...
internal/signal     internal/adb     internal/webrtc（可平行開發）
    ↓                   ↓                 ↓
                ...
                internal/cli
                    ↓
cmd/radb-signal   cmd/radb-agent   cmd/radb
```
