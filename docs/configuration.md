...
| Flag | 環境變數 | 預設值 | 說明 |
|------|---------|--------|------|
| `--port` | `RADB_SERVER_PORT` | `8080` | HTTP/WebSocket 監聽埠 |
| `--host` | `RADB_SERVER_HOST` | `0.0.0.0` | 監聽地址 |

### 啟動範例

```bash
# 最簡啟動
RADB_TOKEN=my-secret radb server

# 指定 port
radb server --port 9090 --token my-secret
```
...
| Flag | 環境變數 | 預設值 | 說明 |
|------|---------|--------|------|
| `--server` | `RADB_SERVER_URL` | `ws://localhost:8080` | Server WebSocket 位址 |
| `--token` | `RADB_TOKEN` | — | PSK Token |
| `--host-id` | `RADB_HOST_ID` | (自動偵測主機名稱) | 註冊在 Server 上的主機識別名稱 |
...
```bash
# 基本啟動
RADB_TOKEN=my-secret radb agent --server ws://server.example.com:8080
...
```bash
# 啟動 daemon
RADB_TOKEN=my-secret radb daemon --server ws://server.example.com:8080
...
# Server
RADB_SERVER_PORT=8080

# Agent / Client
RADB_SERVER_URL=ws://server.example.com:8080
