...
func TestLocalAbstractConnectionOrdering(t *testing.T) {
	...
	openCh := OpenChannelFunc(func(label string) (io.ReadWriteCloser, error) {
		pr, pw := io.Pipe()
		pwCh <- pw
		openCalled <- struct{}{}
...
// TestHandleOPENNonLocalAbstractNonBlocking 驗證非 localabstract 服務（如 tcp:9008）
// 的 OPEN 處理不阻塞主迴圈。
//
// 背景：UIAutomator 透過 tcp:9008 發送大量快速 HTTP 請求。若 handleOPEN 同步呼叫
// openCh（DataChannel 建立），主迴圈在 DC 建立期間無法處理其他 stream 的 OKAY/WRTE/CLSE，
// 導致 UIAutomator 的 HTTP 連線 timeout。
func TestHandleOPENNonLocalAbstractNonBlocking(t *testing.T) {
	// openCh 故意阻塞，模擬 DataChannel 建立耗時
	openBlocked := make(chan struct{})
	openCh := OpenChannelFunc(func(label string) (io.ReadWriteCloser, error) {
		close(openBlocked) // 通知測試：openCh 已被呼叫
		// 阻塞直到 context 取消（模擬 DC 建立耗時）
		select {}
	})
	...
	// handleOPEN 應在 goroutine 中呼叫 openCh，立即返回不阻塞
	returned := make(chan struct{})
	go func() {
		bridge.handleOPEN(ctx, &adbMsg{
			command: aOPEN,
			arg0:    200,
			data:    []byte("tcp:9008\x00"),
		})
		close(returned)
	}()

	// handleOPEN 應在 100ms 內返回（不等待 openCh 完成）
	select {
	case <-returned:
		// 預期行為：handleOPEN 不阻塞
	case <-time.After(500 * time.Millisecond):
		t.Fatal("handleOPEN 阻塞了主迴圈：非 localabstract 的 openCh 不應同步執行")
	}

	// 確認 openCh 確實有被呼叫（在 goroutine 中）
	select {
	case <-openBlocked:
		// openCh 已被呼叫
	case <-time.After(time.Second):
