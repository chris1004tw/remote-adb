...
//
// 相關文件：.claude/CLAUDE.md「Cloudflare TURN 整合」
package gui
...
// 因為 STUN 伺服器由 AppConfig.STUNServer 獨立設定。
//
// 參數：
...
//
// 回傳：
//   - []webrtc.TURNServer：TURN 伺服器清單（通常 1 個，含多個 URL）
//   - error：HTTP 或 JSON 解析錯誤
func fetchCloudflareTURN(ctx context.Context, client *http.Client) ([]webrtc.TURNServer, error) {
	if client == nil {
	...
	// 所有 TURN URL 共用同一組帳密，組成單一 TURNServer（多 URL）
	// 注意：webrtc.TURNServer 只有單一 URL 欄位，所以每個 URL 各建一個
	servers := make([]webrtc.TURNServer, len(turnURLs))
	for i, u := range turnURLs {
		servers[i] = webrtc.TURNServer{
			URL:        u,
